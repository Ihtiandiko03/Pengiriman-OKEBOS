...
@section('container')
    ...
    <form method="post" action="/dashboard/logistik/prosesverifikasi/{{ $item->nomor_resi }}" class="mb-5" enctype="multipart/form-data">
      ...
      <div class="mb-3">
        <label for="berat_barang" class="form-label">Berat Barang dalam gram <b>(Harap Diisi !)</b></label>
        <input type="number" class="form-control @error('berat_barang') is-invalid @enderror" id="berat_barang" name="berat_barang" value="{{ old('berat_barang', $item->berat_barang) }}" >
        @error('berat_barang')
            <div class="invalid-feedback">
                {{ $message }}
            </div>
      ...
      <div class="mb-3">
        <label for="harga" class="form-label">Harga <b>(Harap Diisi !)</b></label>
